...
from core.network_GMA import RAFTGMA
...
def train(args):
    model = nn.DataParallel(RAFTGMA(args))
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                PATH = '/data/csj/meflow_checkpoints/Vitual/GMA/%d_%s_%s.pth' % (total_steps + 1, args.name, args.validation[:2])
                torch.save(model.state_dict(), PATH)
                results = {}
                # for val_dataset in args.validation:
                results.update(evaluate.validate_usdata_all_cubic(model, args))
                print("1",args.validation)
            ...
            if total_steps > args.num_steps:
                ...
                break
    logger.close()
    PATH = '/data/csj/meflow_checkpoints/Vitual/GMA/%s_%s.pth' % (args.name, args.validation)
    torch.save(model.state_dict(), PATH)
    return PATH
...
if __name__ == '__main__':
    ...
    args.validation = 'SAMSUNG MEDISON CO'

    torch.manual_seed(1234)
    np.random.seed(1234)

    if not os.path.isdir('/data/csj/meflow_checkpoints/Vitual/GMA/'):
        os.mkdir('/data/csj/meflow_checkpoints/Vitual/GMA/')
