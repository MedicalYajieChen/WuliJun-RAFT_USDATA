...
def validate_usdata_all_bilinear(model, iters=12):
    ...
    a = (2*len(epe_mask_list)/3)
    for i in range(0, a):
        epe_mask_list[i] = epe_mask_list[i]/k1 
    for i in range(a, len(epe_mask_list)):
    ...
    for i in range(a, len(epe_pointsmask_list)):
    ...
    for i in range(0, a):
       epe_bilinear_list[i] = epe_bilinear_list[i]/k1 
    for i in range(a, len(epe_bilinear_list)):
        epe_bilinear_list[i] = epe_mask_list[i]/k2 
...
@torch.no_grad()
def validate_usdata_all_cubic(model, args, iters=12):
    ...
    for val_id in range(len(val_dataset)):
        ...
        for i in range(len(coordinate_file)):
            ...
            for k in range(len(x)):
                # print(pr_flow_ux.shape, pr_flow_uy.shape)
        ...
        # ['ESAOTE', 'GE Vingmed Ultrasound', 'Hitachi Aloka Medical,Ltd', 'Philips Medical Systems', 'SAMSUNG MEDISON CO', 'Siemens', 'TOSHIBA_MEC_US']
        if(validation == 'ESAOTE'):
            k1 = 4.0584
            k2 = 4.0584
        elif(validation == 'GE Vingmed Ultrasound'):
            k1 = 3.4150
            k2 = 3.4150
        elif(validation== 'Hitachi Aloka Medical,Ltd'):
            k1 = 3.6
            k2 = 3.6
        elif(validation == 'Philips Medical Systems'):
            k1 = 4.0625
            k2 = 3.7531
        elif(validation == 'SAMSUNG MEDISON CO'):
            k1 = 2.58
            k2 = 2.58
        elif(validation == 'Siemens'):
            k1 = 4.2857
            k2 = 4.2857
        elif(validation == 'TOSHIBA_MEC_US'):
            k1 = 3.8154
            k2 = 3.5429
        else:
            print("Error:validation is valid!")
...
@torch.no_grad()
def validate_usdata_all_cubic_ba(model, iters=4):
    model.eval()
    val_dataset = datasets_US_accuracy.USdata(test=True)
    epe_mask_list = []
    epe_pointsmask_list = []
    ...
    for val_id, data_blob in enumerate(val_dataloader):
        ...
        image1, image2, flow_gt, valid_gt, pointsmask, coordinate = data_blob
        # image1 = image1[None].to(device)
        # image2 = image2[None].to(device)
        # image1 = img1.copy()
        # image2 = img2.copy()
        ...
        for b in range(image1.shape[0]):
            # 计算mask epe
            mask = valid_gt[b] > 0
            epe = torch.sum((flow[b] - flow_gt[b]) ** 2, dim=0).sqrt()
            # print(epe[:, mask].mean().item())
            epe_mask_list.append(epe[mask].mean().item())
            # print(flow.shape, flow_gt.shape, epe.shape)

            # 计算pointsmask epe
            points_mask = pointsmask[b] > 0
            epe_pointsmask_list.append(epe[points_mask].mean().item())

            # 计算cubic插值epe
            coordinate_file = coordinate[b]
            pr_flow = flow[b]
            pr_flow_ux = pr_flow[0, :, :]
            pr_flow_uy = pr_flow[1, :, :]
