...
# from core.raft_motion_agg import RAFT
from core.raft_sam import RAFT
...
def fetch_optimizer(args, model):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[1000, 4000, 8000, 12000, 15000, 20000], gamma=0.5)
...
def train(args):
    ...
    if args.restore_ckpt is not None:
        ...
        encoder_check = '/data/csj/GLS/checkpoints/pre_train/SAMUS_MYO.pth'
        model.load_state_dict(torch.load(encoder_check), strict=False)
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                PATH = '/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_samcon/%d_%s_sam+con_twins.pth' % (total_steps + 1, args.name)
            ...
            if total_steps > args.num_steps:
                ...
                break
    logger.close()
    PATH = '/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_samcon/%s_sam+con_twins.pth' % args.name
    torch.save(model.state_dict(), PATH)
    return PATH
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, nargs='+', default=[320, 320])
    ...
    if not os.path.isdir('/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_samcon/'):
        os.mkdir('/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_samcon/')
