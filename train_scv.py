...
from scv.sparsenet import SparseNet
...
def train(args):
    model = nn.DataParallel(SparseNet(args))
    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                PATH = '/data/csj/meflow_checkpoints/Vitual/SCV/%d_%s_%s.pth' % (total_steps + 1, args.name, args.validation[:2])
            ...
            if total_steps > args.num_steps:
                ...
                break
    logger.close()
    PATH = '/data/csj/meflow_checkpoints/Vitual/SCV/%s_%s.pth' % (args.name, args.validation)
    torch.save(model.state_dict(), PATH)
    return PATH
...
if __name__ == '__main__':
    ...
    args.validation = 'ESAOTE'

    torch.manual_seed(1234)
    np.random.seed(1234)

    if not os.path.isdir('/data/csj/meflow_checkpoints/Vitual/SCV/'):
        os.mkdir('/data/csj/meflow_checkpoints/Vitual/SCV/')
