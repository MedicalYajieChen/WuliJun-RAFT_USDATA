...
from core.utils.common import LayerNorm2d, MLPBlock, Adapter, AugAdapter
import math
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.post_pos_embed = PostPosEmbed(embed_dim=embed_dim, ori_feature_size=[1024 // 16, 1024//16],
                                               new_feature_size=[img_size[0] // patch_size, img_size[1] // patch_size])  # new to sam
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.pos_embed is not None:
            ...
            x = x + F.interpolate(self.pos_embed.permute(0,3,1,2), size=(x.shape[1], x.shape[2]), mode='bilinear', align_corners=False).permute(0,2,3,1)
...
class AugBlock(nn.Module):
    ...
    def __init__(
        ...
        # ------------------ new to sam----------------------
        if window_size == 0:
            self.MLP_Adapter = AugAdapter(dim, skip_connect=False,
                                          num_heads=num_heads)  # new to sam, MLP-adapter, no skip connection
...
class AugAttention(nn.Module):
    ...
    def forward(self, x, return_aug=False):
        ...
        x = self.proj(x)
        if return_aug:
            important_key = torch.sum(attn, dim=1).view(B, self.num_heads, H, W).permute(0, 2, 3,
                                                                                         1)  # (b*nHead 1 H*w) -> (b H,  nHead)
...
class PostPosEmbed(nn.Module):
    ...
    def __init__(
            self,
            embed_dim: int = 768,
            ori_feature_size= [64,64],
            new_feature_size =[32, 32]
    ) -> None:
        """
        Args:
            embed_dim (int): Patch embedding dimension.
...
class PatchEmbed0(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = F.interpolate(x, (x.shape[2] + 8, x.shape[3] + 8), mode="bilinear", align_corners=False)##改
        x = self.proj(x)
        # B C H W -> B H W C
        x = x.permute(0, 2, 3, 1)
        return x
...
if __name__ == '__main__':
    img = torch.randn([4,3,368,464])
    encoder = ImageEncoderViT(img_size=[img.shape[2], img.shape[3]])
    output = encoder(img)
    print('1')
