...
class FlowAugmentor:
    ...
    def gamma_augment(self, img1, img2):
        gamma = random.uniform(0.5, 1.5)

        # 进行伽马强度转换
        img1_gamma = np.power(img1 / float(np.max(img1)), gamma)
        img1_gamma = np.uint8(img1_gamma * 255)

        img2_gamma = np.power(img2 / float(np.max(img2)), gamma)
        img2_gamma = np.uint8(img2_gamma * 255)
        return img1_gamma, img2_gamma

    def __call__(self, img1, img2, flow):
        img1, img2 = self.color_transform(img1, img2)
        img1, img2 = self.eraser_transform(img1, img2)
        # img1, img2 = self.gamma_augment(img1, img2)
