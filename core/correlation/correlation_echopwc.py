...
def cupy_kernel(strFunction, objVariables):
    ...
    while True:
        ...
        strIndex = ['((' + strArgs[intArg + 1].replace('{', '(').replace('}', ')').strip() + ')*' + str(
            intStrides[intArg] if torch.is_tensor(intStrides[intArg]) == False else intStrides[intArg].item()) + ')' for
...
class _FunctionCorrelation(torch.autograd.Function):
    ...
    def forward(self, one, two):
        # rbot0 = one.new_zeros([ one.shape[0], one.shape[2] + 3, one.shape[3] + 3, one.shape[1] ])
        # rbot1 = one.new_zeros([ one.shape[0], one.shape[2] + 3, one.shape[3] + 3, one.shape[1] ])
        #
        # one = one.contiguous(); assert(one.is_cuda == True)
        # two = two.contiguous(); assert(two.is_cuda == True)
        #
        # output = one.new_zeros([ one.shape[0], 16, one.shape[2], one.shape[3] ])
        size = 3
        rbot0 = one.new_zeros([one.shape[0], one.shape[2] + size, one.shape[3] + size, one.shape[1]])
        rbot1 = one.new_zeros([one.shape[0], one.shape[2] + size, one.shape[3] + size, one.shape[1]])
        ...
        channel = (size+1)**2
        output = one.new_zeros([one.shape[0], channel, one.shape[2], one.shape[3]])
    ...
    def backward(self, gradOutput):
        ...
        gradOne = one.new_zeros([one.shape[0], one.shape[1], one.shape[2], one.shape[3]]) if self.needs_input_grad[
                                                                                                 0] == True else None
        gradTwo = one.new_zeros([one.shape[0], one.shape[1], one.shape[2], one.shape[3]]) if self.needs_input_grad[
                                                                                                 1] == True else None
            ...
            if gradOne is not None:
                for intSample in range(one.shape[0]):
                    ...
                    cupy_launch('kernel_Correlation_updateGradOne', cupy_kernel('kernel_Correlation_updateGradOne', {
                        ...
                        block=tuple([512, 1, 1]),
                        args=[cupy.int32(n), intSample, rbot0.data_ptr(), rbot1.data_ptr(), gradOutput.data_ptr(),
                              gradOne.data_ptr(), None]
                    )
                # end
            # end
            ...
            if gradTwo is not None:
                for intSample in range(one.shape[0]):
                    ...
                    cupy_launch('kernel_Correlation_updateGradTwo', cupy_kernel('kernel_Correlation_updateGradTwo', {
                        ...
                        grid=tuple([int((n + 512 - 1) / 512), 1, 1]),
                        block=tuple([512, 1, 1]),
                        args=[cupy.int32(n), intSample, rbot0.data_ptr(), rbot1.data_ptr(), gradOutput.data_ptr(), None,
                              gradTwo.data_ptr()]
                    )
                # end
            # end
