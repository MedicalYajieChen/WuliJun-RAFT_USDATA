...
from core.utils.augmentor_mask import FlowAugmentor, SparseFlowAugmentor
import cv2
import joblib
...
class FlowDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_test:
            ...
            ##seg
            (h,w) = seg_mask_1.shape
            pad_ht = (((h // 8) + 1) * 8 - h) % 8
            pad_wd = (((w // 8) + 1) * 8 - w) % 8
            H = h + pad_ht
            W = w + pad_wd
            seg_mask_1 = torch.from_numpy(seg_mask_1).float()
            seg_mask_1 = F.pad(seg_mask_1, [pad_wd//2, pad_wd - pad_wd//2], mode='replicate')
            seg_mask_1 = cv2.resize(seg_mask_1.numpy(), [W//8, H//8])
            seg_mask_1 = torch.from_numpy(seg_mask_1).unsqueeze(0).float()

            img1 = torch.from_numpy(img1).permute(2, 0, 1).float()
            img2 = torch.from_numpy(img2).permute(2, 0, 1).float()
            return img1, img2, flow, valid, pointsmask, coordinate, seg_mask_1
        ...
        # seg_mask_2 = frame_utils.read_gray_img(self.train_seg_mask_list[index][end_index])
        # 转成类别标签，心室为2，心肌层为1，其余为0
        seg_mask_1[seg_mask_1 > 200] = 2
        seg_mask_1[seg_mask_1 > 100] = 1
        ...
        if self.augmentor is not None:
            if self.sparse:
                img1, img2, flow, valid, seg_mask_1 = self.augmentor(img1, img2, flow, valid, seg_mask_1)
            else:
                img1, img2, flow, seg_mask_1 = self.augmentor(img1, img2, flow, seg_mask_1)

        img1 = torch.from_numpy(img1).permute(2, 0, 1).float()
        img2 = torch.from_numpy(img2).permute(2, 0, 1).float()
        flow = torch.from_numpy(flow).permute(2, 0, 1).float()
        (H, W) = img1.shape[1:]
        seg_mask_1 = cv2.resize(seg_mask_1, [W//8, H//8])
        # print('train:'+str(seg_mask_1.shape))
        seg_mask_1 = torch.from_numpy(seg_mask_1).unsqueeze(0).float()
        ...
        return img1, img2, flow, valid.float(), seg_mask_1
...
class USdata(FlowDataset):
    def __init__(self, aug_params=None, test=False, root='/home/csj/data/data_accuracy/'):
        super(USdata, self).__init__(aug_params, sparse=True)
        validation = 'SAMSUNG MEDISON CO'
        train_images_list, train_flow_list, train_mask_list, test_images_list, test_flow_list, test_mask_list, test_pointsmask_list, test_coordinate_list, train_seg_mask_list, test_seg_mask_list = get_USData_list(root, validation)
...
def get_USData_list(root_path, validation):
    ...
    suppliers = ['ESAOTE', 'GE Vingmed Ultrasound', 'Hitachi Aloka Medical,Ltd', 'Philips Medical Systems', 'Siemens', 'SAMSUNG MEDISON CO', 'TOSHIBA_MEC_US']
    train_images_list = []
    train_flow_list = []
    train_mask_list = []
    test_images_list = []
    ...
    for sp in suppliers:
        if sp == validation:
            images_list, flow_list, mask_list, pointsmask_list, coordinate_list, seg_mask_list = get_subset_list(root_path, sp, test=True)
    ...
    return train_images_list, train_flow_list, train_mask_list, test_images_list, test_flow_list, test_mask_list, test_pointsmask_list, test_coordinate_list, train_seg_mask_list, test_seg_mask_list
...
def get_subset_list(root_path, supplier, test=False):
    ...
    if test:
        return images_list, flow_list, mask_list, pointsmask_list, coordinate_list, seg_mask_list
    else:
