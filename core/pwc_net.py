...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        class Refiner(torch.nn.Module):
            def __init__(self):
                ...
                self.netMain = torch.nn.Sequential(
                    ...
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
                    torch.nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=2,
                                    dilation=2),
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
                    torch.nn.Conv2d(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=4,
                                    dilation=4),
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
                    torch.nn.Conv2d(in_channels=128, out_channels=96, kernel_size=3, stride=1, padding=8,
                                    dilation=8),
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
                    torch.nn.Conv2d(in_channels=96, out_channels=64, kernel_size=3, stride=1, padding=16,
                                    dilation=16),
                    torch.nn.LeakyReLU(inplace=False, negative_slope=0.1),
        ...
        self.load_state_dict({strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in
                              torch.hub.load_state_dict_from_url(
                                  ...
                                  file_name='pwc-' + 'chairs-things').items()})
    ...
    def forward(self, tenOne, tenTwo):
        ...
        flow = torch.nn.functional.interpolate(flow, size, mode='bilinear', align_corners=False)

        return flow
if __name__ == '__main__':
    model = Network().cuda()
    img1 = torch.randn([1, 3, 384, 448]).cuda()
    img2 = torch.randn([1, 3, 384, 448]).cuda()
