...
class ConvNext(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0):
       ...
       self.backbone = timm.create_model('convnext_tiny_384_in22ft1k', pretrained=True, features_only=True, in_chans=3)
       self.chan_tran = nn.Sequential(
           nn.Conv2d(192, output_dim, kernel_size=3, stride=1, padding=1),
           # nn.Conv2d(1024, 320, kernel_size=3, stride=1, padding=1),
...
class BasicEncoderQuarter(nn.Module):
    def __init__(self, output_dim=128, norm_fn='batch', dropout=0.0):
        ...
        self.layer1 = self._make_layer(64,  stride=1)
        self.layer2 = self._make_layer(96, stride=2)
        self.layer3 = self._make_layer(128, stride=1)##quarter
    ...
    def _make_layer(self, dim, stride=1):
        layer1 = ResidualBlockQuarter(self.in_planes, dim, self.norm_fn, stride=stride)
...
class ResidualBlockQuarter(nn.Module):
    ...
    def forward(self, x):
        # print('r0:' + str(x.shape))
        y = x
        y = self.relu(self.norm1(self.conv1(y)))
        y = self.relu(self.norm2(self.conv2(y)))
        # print('r1:' + str(y.shape))

        if self.downsample is not None:
            x = self.downsample(x)
        # print('r2:' + str(x.shape))
