...
class FlowDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_test:
            ...
            valid = cv2.resize(valid, [Height, Width])
            valid = torch.from_numpy(valid)

            # 读取pointsmask
            pointsmask = frame_utils.read_gray_img(self.test_pointsmask_list[index])
            pointsmask = cv2.resize(pointsmask, [Height, Width])
        ...
        # if img1.shape[1]<=376:
        #     print('1')
        if self.image_list[index][0]=='/data/csj/data_accuracy/TMI_DATA_pro/TMI_DATA_with_artifacts/patient077/frames/00001.png':
            print('1')
        ...
        img2 = torch.from_numpy(img2).permute(2, 0, 1).float()
        flow = torch.from_numpy(flow).permute(2, 0, 1).float()
        if img1.shape[1]!=352 or img2.shape[1]!=352 or flow.shape[1]!=352 or valid.shape[0]!=352:
            print('3')
        if img1.shape[2]!=376 or img2.shape[2]!=376 or flow.shape[2]!=376 or valid.shape[1]!=376:
            print('4')
...
def get_USData_list(root_path, validation):
    ...
    suppliers = ['ESAOTE', 'GE Vingmed Ultrasound', 'Hitachi Aloka Medical,Ltd', 'Philips Medical Systems',
                 'SAMSUNG MEDISON CO', 'Siemens', 'TOSHIBA_MEC_US']
    train_images_list = []
    train_flow_list = []
    train_mask_list = []
    test_images_list = []
...
def get_subset_list(root_path, supplier, test=False):
    ...
    view_name = os.listdir(os.path.join(root_path, supplier))
    ...
    coordinate_list = []
    for vn in view_name:
        mode_name = os.listdir(os.path.join(root_path, supplier, vn))
        for mn in mode_name:
