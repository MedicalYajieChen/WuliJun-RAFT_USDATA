    ...
    while should_keep_training:
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % VAL_FREQ == VAL_FREQ - 1:
                PATH = '/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_msraft/%d_%s.pth' % (total_steps + 1, args.name)
            ...
            if total_steps > args.num_steps:
                ...
                break
    logger.close()
    PATH = '/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_msraft/%s.pth' % args.name
    torch.save(model.state_dict(), PATH)
    return PATH
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', type=int, nargs='+', default=[368, 464])
    ...
    args.restore_ckpt = 'models/sintel_msraft.pth'

    torch.manual_seed(1234)
    np.random.seed(1234)

    if not os.path.isdir('/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_msraft/'):
        os.mkdir('/data/csj/GLS/checkpoints/RAFT_USDATA/checkpoints_msraft/')
